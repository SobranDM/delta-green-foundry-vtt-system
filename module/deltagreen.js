...
function changeTouchesAdaptations(change) {
...
function changeTouchesSanity(change) {
...
Hooks.on("preUpdateActor", (actor, change) => {
  ...
  if (changeTouchesSanity(change)) {
    const { value, currentBreakingPoint } = actor.system.sanity ?? {};
    if (typeof value === "number")
      sanityValueBeforeUpdate.set(actor.id, {
        value,
        aboveBreakingPoint:
  ...
  if (!changeTouchesAdaptations(change)) return;
  const { adaptations } = actor.system.sanity ?? {};
  if (!adaptations) return;
  adaptationStateBeforeUpdate.set(actor.id, {
...
Hooks.on("updateActor", async (actor, _change) => {
  ...
  const adaptationBefore = adaptationStateBeforeUpdate.get(actor.id);
  adaptationStateBeforeUpdate.delete(actor.id);
  const sanityBefore = sanityValueBeforeUpdate.get(actor.id);
  sanityValueBeforeUpdate.delete(actor.id);
